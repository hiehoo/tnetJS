...
export async function handleStart(ctx: Context): Promise<void> {
  ...
  try {
    ...
    const messageText = ctx.message && 'text' in ctx.message ? ctx.message.text : '/start';
 ...
 */
export async function handleStartWithEntryPoint(ctx: Context, entryPoint: EntryPoint): Promise<void> {
  if (!ctx.from) return;
  ...
  try {
    ...
    try {
      ...
      console.log(`Creating new user for ID ${userId} from start command with entry point ${entryPoint}`);
    ...
    console.error(`Error in start command with entry point ${entryPoint}:`, error);
    await ctx.reply('Welcome to TNETC Trading! Please try using the /start command again.');
  }
} 