...
import { database, testimonialService, statsService } from "../services";
    ...
    if (testimonials.length > 0) {
      ...
      setTimeout(async () => {
        try {
          ...
          await database.updateUser(userId, {
            testimonialsSent:
              (user.testimonialsSent || 0) + testimonials.length,
 ...
 */
export async function handleServiceResults(
  ctx: Context,
  service: ServiceType
): Promise<void> {
  if (!ctx.from) return;
    ...
    if (resultImages.length > 0) {
      ...
      for (const imagePath of resultImages) {
        if (imagePath.includes(`${service}_results`)) {
          ...
          imagePath.includes("x10_results")
        ) {
          availableImages.push(imagePath);
        }
      }
      ...
      for (let i = 0; i < availableImages.length; i += 3) {
        ...
        const mediaGroup = batch.map((imagePath, index) => ({
          ...
          caption:
            index === 0
              ? `Outstanding performance with our ${service} service!`
              : "",
          parse_mode: "Markdown" as const,
        }));
 ...
 */
export async function handleHowItWorks(
  ctx: Context,
  service: ServiceType
): Promise<void> {
  if (!ctx.from) return;
 ...
 */
export async function handleServicePricing(
  ctx: Context,
  service: ServiceType
): Promise<void> {
  if (!ctx.from) return;
